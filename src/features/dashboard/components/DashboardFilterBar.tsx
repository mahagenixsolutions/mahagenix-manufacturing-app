...
export default function DashboardFilterBar() {
  ...
  return (
    <div className="bg-surface border-y border-subtle py-3 px-4 md:px-6 flex items-center justify-between sticky top-0 z-20 shadow-sm backdrop-blur-md bg-opacity-95 overflow-x-auto no-scrollbar w-full">
      <div className="flex items-center gap-4 min-w-max">
        ...
        <div className="flex items-center gap-3">
          {/* Plant */}
          <div className="flex items-center gap-2 bg-surface-secondary border border-subtle rounded-md px-3 py-1.5 text-[11px] lg:text-[13px] hover:border-primary-300 transition-all cursor-pointer">
            <Factory className="w-3.5 h-3.5 text-tertiary" />
            <select
              value={plant}
              onChange={(e) => setPlant(e.target.value)}
              className="bg-transparent text-primary outline-none cursor-pointer font-medium"
            >
