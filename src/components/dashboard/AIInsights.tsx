...
import { Sparkles, TrendingDown, AlertTriangle, ArrowUpRight, Wrench, BarChart3, ChevronRight, Activity } from 'lucide-react';
            ...
            >
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-center gap-3">
                  <div className={`w-7 h-7 rounded flex items-center justify-center shrink-0 ${insight.bg} ${insight.color}`}>
                    <Icon className="w-4 h-4" />
                  </div>
                  <span className="text-[12px] font-bold text-primary">{insight.title}</span>
                </div>
                <div className="flex items-center gap-1 text-[10px] font-semibold text-primary-600 bg-primary-50 px-1.5 py-0.5 rounded shrink-0">
                  <Sparkles className="w-2.5 h-2.5" /> {insight.confidence}% Match
                </div>
              </div>
              ...
              <div className="bg-surface-secondary/50 p-2.5 rounded flex items-start gap-2 border border-subtle border-dashed mt-4">
                 ...
                 <div>
                   <span className="text-[9px] font-bold text-tertiary uppercase tracking-wider block mb-0.5">Root Cause</span>
                   <span className="text-[11px] font-medium text-secondary">{insight.rootCause}</span>
                 </div>
              </div>
