...
const categoryConfig: Record<string, any> = {
  'machine-failure': { icon: Cpu, color: 'text-danger-500', bg: 'bg-danger-500/15' },
  'quality-issue': { icon: ShieldCheck, color: 'text-warning-500', bg: 'bg-warning-500/15' },
  'low-inventory': { icon: Warehouse, color: 'text-warning-600', bg: 'bg-warning-600/15' },
  'maintenance-due': { icon: Wrench, color: 'text-primary-500', bg: 'bg-primary-500/15' },
  'safety': { icon: AlertOctagon, color: 'text-danger-600', bg: 'bg-danger-600/15' },
};
...
export default function RecentAlerts() {
  ...
  const handleAction = (id: string, action: 'acknowledge' | 'resolve' | 'escalate') => {
    setAlerts(prev => prev.map(alert => {
      if (alert.id === id) {
        if (action === 'acknowledge') return { ...alert, status: 'acknowledged', read: true };
  ...
  const activeAlerts = alerts.filter(a => a.status !== 'resolved').slice(0, 8);

  return (
    <div className="flex flex-col h-full">
      <div className="space-y-4 overflow-y-auto no-scrollbar">
        {activeAlerts.map((alert) => {
          const config = categoryConfig[alert.category] || categoryConfig['machine-failure'];
          ...
          return (
            <div
              ...
              className={`group flex flex-col p-5 rounded-lg border border-subtle transition-all cursor-pointer ${
                alert.status === 'new' ? 'bg-surface hover:bg-surface-hover' : 'bg-surface-secondary/50 hover:bg-surface-secondary'
              }`}
            >
              <div className="flex items-start gap-4">
                <div className={`w-10 h-10 rounded-lg flex items-center justify-center shrink-0 mt-0.5 ${config.bg} ${config.color}`}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3">
                    <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded uppercase ${severityColors[alert.severity]}`}>
                      {alert.severity}
                    </span>
              ...
              <div className="mt-2 pt-2 border-t border-subtle flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                <button 
                  onClick={(e) => { e.stopPropagation(); handleAction(alert.id, 'escalate'); }}
